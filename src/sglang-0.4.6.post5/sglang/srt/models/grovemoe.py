...
class Qwen3MoeSparseMoeBlock(nn.Module):
    ...
    def forward_normal(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        small_experts_hidden_states = fused_experts(hidden_states, self.w1, self.w2, routing_weights, group_selected_experts, inplace=False)

        large_experts_hidden_states = self.experts(
            hidden_states=hidden_states, router_logits=router_logits
        )

        # if self.layer_id == 0 and large_experts_hidden_states.shape[0] != 1:
        #     print("large", large_experts_hidden_states)
        #     print("small", small_experts_hidden_states)
...
class Qwen3MoeDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        # if self.layer_id < 2:
        #     print(self.layer_id, hidden_states)
    ...
    def load_weights(self, weights: Iterable[Tuple[str, torch.Tensor]]):
        ...
        for name, loaded_weight in weights:
            ...
            if "mlp.chunk_experts." in name:
                # 这部分逻辑是从 V2 版本借鉴并简化的
                processed = False
                # 尝试应用堆叠映射（例如 gate_proj + up_proj -> gate_up_proj）
        ...
        for i in range(48):
            self.model.layers[i].mlp.pack_params()
